...
def main():
    ...
    #plot_channelstats_sliders(stats_df, iec_stats_df)
    #plot_channel_ws_overview_sliders(stats_df, iec_stats_df)
    #plot_contours_sliders(stats_df)
    
    """if selected_study == "cnbl":
        choice = input("\nWant to see awesome joint probability plots? [y/n]")
        if choice.strip() == "y":
            ch_choice = int(input("\nEnter the channel you want to assess: "))
            plot_joint_probabilities(stats_df, ch_choice)"""
    ...
    #plot_del_ws_overview(all_ws_filtered_df, iec_stats_df)
