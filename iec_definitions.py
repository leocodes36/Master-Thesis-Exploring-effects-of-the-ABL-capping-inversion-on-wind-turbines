...
def get_strength():      # According to IEC-61400-1:2019 Annex C.2 Eq.(C.12)
    return 1

def get_anisotropy():           # According to IEC-61400-1:2019 Annex C.2 Eq.(C.12)
    return 3.9

def get_sigma_1_iec(U, TI):         # According to IEC-61400-1:2019 6.3.2.3 Eq.(10)
    return TI * (0.75 * U + 5.6)

def get_sigma_1_mann(L, alphaepsilon):
    sigma_1 = sqrt(0.69*alphaepsilon*pow(L,2.0/3.0))*1.80277563773199464654
    return sigma_1
