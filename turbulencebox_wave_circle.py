...
from common import *
from constraint import constraint_wave_circ
...
# Reference point, circle will be placed here
yr, zr = 90, 160
...
# --- Figure 3 ---
# Plot a side view of the turbulence at reference yr plane before constraint
plt.figure()
...
constraints = constraint_wave_circ((yr, zr), 2, U, params["Nxyz"], params["dxyz"], 290, 0.003)

# --- Figure 5 ---
# Plotting "Raw turbulence" before constraining
axes = plt.subplots(3, 1)[1]
for ax, c in zip(axes, 'uvw'):
    da.sel(uvw=c, y=yr, z=yr).plot(ax=ax, label='Raw turbulence')

# Use .constrain() method of the mtf object
mtf.constrain(constraints)

# Plot "Constraints" (sine wave with Nc) and "Constrained turbulence"
for i, (ax, c) in enumerate(zip(axes, 'uvw'), 3):
    mtf.to_xarray().sel(uvw=c, y=yr, z=zr).plot(ax=ax, label='Constrained turbulence')

    ax.plot([], '.k', label='Constraints')
    constraints_filtered = [con for con in constraints if con[1] == yr and con[2] == zr]
...
# --- Figure 8 ---
# Plot a time series at yr, zr and one out before and after constraint
plt.figure()
da.sel(uvw='u', y=yr, z=zr).plot(x='x', label='center before constraint')
da1.sel(uvw='u', y=yr, z=zr).plot(x='x', label='center after constraint')
da.sel(uvw='u', y=yr+1, z=zr+1).plot(x='x', label='r+1 before constraint')
da1.sel(uvw='u', y=yr+1, z=zr+1).plot(x='x', label='r+1 after constraint')
plt.grid()
plt.legend()

make_plots("wave_circ")