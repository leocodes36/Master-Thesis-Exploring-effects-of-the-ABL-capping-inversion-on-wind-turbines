...
if ls "$HTC_DIR"/*.htc 1> /dev/null 2>&1; then
    ...
    sbatch /home/s244289/dlbs/cnbl_build_htc_files.sh
fi
...
generate_w_sequence() {
    ...
    python3 -c "h = $h; w_range = list(map(float, '$w_range'.split())); \
                ...
                for W in $(generate_w_sequence $H); do
                    for S in $(generate_sequence $S_RANGE); do
                        for SEED in $(generate_sequence $SEED_RANGE); do
                            echo "$WS $TI $H $W $S $SEED" >> "$PARAM_FILE"
                        done
                    done
                done
            done
...
JOB_ID=$(sbatch --array=0-$(($CHUNK_SIZE - 1))%$MAX_PARALLEL_JOBS \
    ...
    cnbl_build_turb_box.sh | awk '{print $4}')
