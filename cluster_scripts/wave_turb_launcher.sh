...
if ls "$HTC_DIR"/*.htc 1> /dev/null 2>&1; then
    ...
    sbatch /home/s244289/dlbs/wave_build_htc_files.sh
fi
                    ...
                    for H in $H_RANGE; do
                        for W in $W_RANGE; do
                            for SEED in $(generate_sequence "$SEED_RANGE"); do
                                echo "$WS $TI $N $A $H $W $SEED" >> "$PARAM_FILE"
                            done
                        done
                    done
                done
...
JOB_ID=$(sbatch --array=0-$(($CHUNK_SIZE - 1))%$MAX_PARALLEL_JOBS \
    ...
    --nodes=1 \
    --exclusive \
    wave_build_turb_box.sh | awk '{print $4}')
