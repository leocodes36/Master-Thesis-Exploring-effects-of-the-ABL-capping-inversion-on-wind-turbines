...
read WS TI N A H W SEED < <(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" "$PARAM_FILE")

DX=$(echo "${WS}*${TIME_STOP}/${NX}" | bc -l)
TAG="${DLB_NAME}_ws_${WS}_ti_${TI}_n_${N}_a_${A}_h_${H}_w_${W}_seed_"

python /home/s244289/py/dlb_generator/generate_box_wave.py "$WS" "$TI" "$TIME_STEP" "$NX" "$NY" "$NZ" "$DX" "$DY" "$DZ" "$HF" "$TAG" "$SEED" "$N" "$A" "$H" "$W"
