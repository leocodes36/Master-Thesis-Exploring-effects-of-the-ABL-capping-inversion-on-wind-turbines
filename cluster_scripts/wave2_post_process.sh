    ...
    for TI in $(generate_sequence "$TI_RANGE"); do
        for N in $N_RANGE; do
            for A in $A_RANGE; do
                ...
                RES_LIST="/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_n_${N}_a_${A}.lst"
                RES_LINE=""
                RES_CNT=0
                ...
                getChannelStatistics "${RES_LIST}" 7 ${SENS_LIST} -SQF > "/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_n_${N}_a_${A}.stats"
            done
        done
    done
done
    ...
    for TI in $(generate_sequence "$TI_RANGE"); do
        for N in $N_RANGE; do
            for A in $A_RANGE; do
                ...
                HIST_FILE="/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_n_${N}_a_${A}.hist"

                for SENS in ${SENS_LIST}; do

                    # Echo param combo into .hist file
                    echo "${DLB_NAME}_ws_${WS}_ti_${TI}_n_${N}_a_${A}_channel_${SENS}" >> "$HIST_FILE"
