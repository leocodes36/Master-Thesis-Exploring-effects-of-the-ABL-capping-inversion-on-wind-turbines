...
#SEED_RANGE="1:1:10"
WS_RANGE="4.0:2.0:6.0"
TI_RANGE="0.16:0.0:0.16"
SHEAR_RANGE="0.1:0.0:0.1"
SEED_RANGE="1:1:2"
...
for WS in $(generate_sequence "$WS_RANGE"); do
    for SHEAR in $(generate_sequence "$SHEAR_RANGE"); do
        for TI in $(generate_sequence "$TI_RANGE"); do
            ...
            RES_LIST="/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_shear_${SHEAR}.lst"
            RES_LINE=""
            RES_CNT=0
            ...
            for SEED in $(generate_sequence "$SEED_RANGE"); do
                ...
                TAG="${DLB_NAME}_ws_${WS}_ti_${TI}_shear_${SHEAR}_seed_${SEED}"
            ...
            getChannelStatistics "${RES_LIST}" 7 ${SENS_LIST} -SQF > "/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_shear_${SHEAR}.stats"
...
for WS in $(generate_sequence "$WS_RANGE"); do
    for SHEAR in $(generate_sequence "$SHEAR_RANGE"); do
        for TI in $(generate_sequence "$TI_RANGE"); do
            ...
            HIST_FILE="/home/s244289/dlbs/${DLB_NAME}/post/${DLB_NAME}_ws_${WS}_ti_${TI}_shear_${SHEAR}.hist"

            for SENS in ${SENS_LIST}; do

                # Echo param combo into .hist file
                echo "${DLB_NAME}_ws_${WS}_ti_${TI}_shear_${SHEAR}_channel_${SENS}" >> "$HIST_FILE"
